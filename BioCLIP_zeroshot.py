...
from marineDataset import MarineSpeciesDataset
...
underwater_templates = [
    lambda c: f"an underwater photo of a {c}.",
    lambda c: f"a deep-sea image of a {c}.",
    lambda c: f"a marine species named {c}.",
    lambda c: f"a remotely captured image of a {c} in the ocean.",
    lambda c: f"a camera trap image of a {c} on the seafloor.",
    lambda c: f"an underwater photo showing {c} in its habitat.",
    lambda c: f"a low-light deep-sea photo of a {c}.",
    lambda c: f"a ROV (remotely operated vehicle) image of a {c}.",
    lambda c: f"a marine biology research image of {c}.",
    lambda c: f"an image of {c} taken with an underwater camera.",
]
...
def evaluate(model, data, args):
    ...
    for split in data:
        ...
        txt_features = make_txt_features(model, classnames, underwater_templates, args)
...
if __name__ == "__main__":
    ...
    data["train"] = torch.utils.data.DataLoader(
        MarineSpeciesDataset(csv_file = f'{dataset_path}/annotations.csv', image_dir = f'{dataset_path}/rois', transform=preprocess_val),
        batch_size=args.batch_size,
        num_workers=args.workers,
        sampler=None,
        shuffle=False,
